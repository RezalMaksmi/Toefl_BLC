...
import { postAPIAct } from "../../redux/fetch/Get";
import { useDispatch, useSelector } from "react-redux";
import axios from 'axios';

const AddDataPeserta = (props) => {
  ...
  const addDataPeserta = async () => {
     const dataPeserta = {
      ...
      if( !dataPeserta.no_reg||!dataPeserta.alamat||!dataPeserta.instansi||!dataPeserta.tgl_lahir||!dataPeserta.gender ||!dataPeserta.no_hp|| !dataPeserta.role_kelas || !dataPeserta.jenis_peserta||!dataPeserta.nama||!dataPeserta.email){
        setErrorMessage("Data tidak boleh kosong!")
        setMessage(true)
        return;
      }else if(dataPeserta.no_reg === "948569345934759857"){
        setErrorMessage("Data peserta sudah terdaftar!")
        setMessage(true)
        return;
      }else {
      
    
        // fetch('http://localhost:8000/peserta', {
        //     method: 'POST',
        //     headers: {
        //         "Content-Type": "application/json"
        //     },
        //     body: JSON.stringify(dataPeserta)
        //     }).then(() => {
        ...
        // })
        axios.post('http://localhost:8000/peserta', dataPeserta, {
            headers: {
                "Content-Type": "application/json"
            }
        }).then(() => {
    ...
    // try {
    //     await axios.post('http://localhost:8000/peserta/', { dataPeserta });
    //     console.log("berhasil ditambahkan")
    //     // setNewData('');
    //     // clearInterval(intervalId); // Clear interval once after adding new data
    //     // fetchData(); // Fetch data again to update the list
    //   } catch (error) {
    //     console.error('Error adding new data:', error);
    //   }
    
        // console.log(JSON.stringify(dataPeserta))
        //   try {
        //     dispatch(postAPIAct(`http://localhost:8000/peserta/`, {
